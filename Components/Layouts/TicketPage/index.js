  ...
  return (
    <div style={{minHeight:'50vh', backgroundColor:"white"}}>
        ...
        <div className={`${size.width>600?"tickets-cont pb-5":"pb-3 px-5"}`}>
            ...
            {tickets.map((x, i)=>{
            return(
            <Col md={12} xs={12} key={i} className={`${size.width>600?"":"py-0"}`}>
                {i!==0 && <hr className='my-0 py-0' />}
                {x.BookedToursOptions.map((y, j)=>{
                return(
                <>
                {j!=0 &&<hr/>}
                <Row className={y.check?'selected-ticket-row': size.width>600?'ticket-row':"py-3 px-1"} key={"a"+j}
                    onClick={async()=>{
                        setLoad(true);
                ...
                >
                    <Col md={2} xs={12}>
                        <img src={x.image} height={size.width>600?100:150} width={size.width>600?140:"100%"} style={{borderRadius:5}} alt="Tour" />
                    </Col>
                    <Col md={6} className={`${size.width>600?"":"mt-1"}`}>
                    <h5>{y.tourOptName}</h5>
                    <div className='mx-1'>
                    ...
                    <Col md={4} xs={12}>
                        <div style={size.width>600?{float:'right', height:"100%"}:{}}>
                            <div className={`${size.width>600? "text-end":""}`}>
                                ...
                                <img src={'/icons/ticket-available.png'} className={`${size.width>600?"":"mx-2"}`} height={50} alt="Ticket" />
                                </>:
                                <>
                                <div className='mx-3 fs-12'>{y.TourOption.manual?'Not Downloadable':'Downloadable'}</div>
                                <div className='mx-3 fs-18'>Pending</div>
                                <img src={'/icons/ticket-pending.png'} className={`${size.width>600?"":"mx-3"}`} height={50} alt="Pending" />
                                </>
                                }
                                {(y.reviewed=="1" && size.width>600) &&
            ...
            <>
            {!load && <ReactToPrint content={()=>inputRef} trigger={()=><button className={`custom-btn ${size.width>600?"":"mt-3"}`}>Get Ticket</button>} />}
