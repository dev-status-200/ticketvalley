...
export async function getServerSideProps({req, res}){
  const bestSellingData = await axios.get(process.env.NEXT_PUBLIC_GET_PRODUCT_BY_CATEGORY,{
    headers:{ "category": "Best Selling" }
  }).then((x)=>x.data.result)
  const adventureData = await axios.get(process.env.NEXT_PUBLIC_GET_PRODUCT_BY_CATEGORY,{
    headers:{ "category": "Adventure Tours" }
  }).then((x)=>x.data.result)
  const comboData = await axios.get(process.env.NEXT_PUBLIC_GET_PRODUCT_BY_CATEGORY,{
